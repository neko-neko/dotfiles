...
name: spec-review
description: >-
  設計書レビューワークフロー。3つの観点（clarity, feasibility, consistency）で
...
3つの観点で設計書を並列レビューし、統合レポートを生成する。ユーザーが承認した指摘のみを設計書に反映する。
...
3つ（`codex_enabled` が true の場合は4つ）のレビューを **並列** で起動する。すべて `run_in_background: true` を使用し、結果を収集する。

### 2-1. spec-review-clarity

Agent tool を使用する。`subagent_type` に `spec-review-clarity` を指定し、prompt に `doc_path` と `doc_content` を含め、以下の観点でレビューさせる:
...
```bash
codex exec "以下の設計書をレビューしてください。技術的実現可能性、明確さ、一貫性の観点で問題点を指摘してください。
...
```json
{
  ...
  "category": "clarity | feasibility | consistency | codex",
  "description": "指摘内容",
  "suggestion": "改善案"
}
```
...
```bash
codex exec "以下の設計書レビューレポートと設計書を確認し、2つの観点で分析してください。
...
1. findings 全件を JSON 配列に変換する。各 finding に `selected: true/false` を付与:
   ```json
   [
     {"index": 1, "severity": "high", "category": "clarity", "description": "...", "selected": true},
     {"index": 2, "severity": "medium", "category": "feasibility", "description": "...", "selected": false}
   ]
   ```
...
指摘内容と suggestion に基づき、オーケストレーター自身が設計書を直接修正する。修正手順:
