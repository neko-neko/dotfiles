...
### 2-5. codex review（`codex_enabled` 時のみ）
...
1. findings 全件を JSON 配列に変換する。各 finding に `selected: true/false` を付与:
   ```json
   [
     {"index": 1, "severity": "high", "category": "requirements", "description": "...", "selected": true},
...
ユーザーが選択した各 finding について、修正方法を判定する:

#### 判定基準

- **自明な修正** — suggestion の内容だけで修正が完結するもの（文言の具体化、セクションの追加、図表の修正など）。オーケストレーターが設計書を直接修正する
- **ユーザー判断が必要** — category が `requirements` または `design-judgment` の finding で、業務ルールの確認・要件の具体化・設計判断の変更が必要なもの。ユーザーにヒアリングしてから修正する
...
1. finding の description と suggestion をユーザーに提示する
2. 「この指摘について確認させてください: [具体的な質問]」の形式でヒアリングする
3. ユーザーの回答を反映して設計書を修正する
4. 修正内容をユーザーに確認する
