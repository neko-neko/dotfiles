...
予算は参照元チケットのステータスと優先度の組み合わせで決定する:

| 条件 | 予算 | 追加要件 |
|------|------|---------|
| (In Progress または In Review) かつ (Urgent または High) | 800文字 + 生の抜粋 | 最新メッセージは可能な限り全文、言及URLを全列挙、`deferred_signals` を記録 |
| 上記以外の In Progress/In Review、または Urgent/High | 400文字 | `referenced_urls` を別フィールドで保持 |
| その他（Todo/Backlog かつ Medium 以下） | 200文字 | 基本的な要約 |

判定ロジック（擬似コード）:
```
active = status in [In Progress, In Review]
hot    = priority in [Urgent, High]

if active and hot  → 800
elif active or hot → 400
else               → 200
```
