...
- depends_on_artifacts: [artifacts/doc-audit/script-output.json]

### DOC-02: depends-on 未宣言の解消
- severity: blocker
- verify_type: automated
...
### DOC-04: 新規/更新ドキュメントの frontmatter 整備
- severity: blocker
- verify_type: automated
- verification:
  1. doc-audit フェーズで作成/更新された md ファイルを git diff --name-only で列挙
...
- depends_on_artifacts: [artifacts/diff/doc-audit.diff]

### DOC-05: doc-check 実行完了
- severity: blocker
- verify_type: automated
- verification:
  1. doc-check 実行ログを確認
...
- depends_on_artifacts: [artifacts/doc-audit/doc-check.log]
...
- depends_on_artifacts: [artifacts/doc-audit/report.json]
- scope: B のみ
...
- verification:
  ...
  2. 各 finding の status を確認
  3. status が "updated" の場合、対象 doc の git log --format=%at -1 が doc-audit フェーズ開始タイムスタンプ以降であること
  4. status が "skipped" の場合、user_decision フィールドが存在すること
- pass_condition: stale findings 全件の status ∈ {"updated", "skipped"} かつ "updated" の doc は最終コミットが doc-audit フェーズ開始以降
...
### DOC-12: README/CONTRIBUTING/CHANGELOG 整合
- severity: quality
- verify_type: inspection
- verification:
