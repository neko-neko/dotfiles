...
4. Audit Agent を起動:
   ...
   - `--swarm` 有効時 かつ inspection 基準なし（Phase 9 等）: 単一の `phase-auditor`
...
```json
{
  ...
  "args": { "codex": false, "e2e": false, "smoke": true, "iterations": 3, "swarm": false }
}
```
...
`--swarm` フラグ有効時、inspection 基準を含むフェーズ（Phase 1-8）の Audit Gate をエージェントチームで実行する。automated 基準のみのフェーズ（Phase 9）は単一エージェントのまま。
...
| Phase 3 | 3（D3-02, D3-03, D3-04） | Yes |
| Phase 4 | 2（D4-03, D4-04） | Yes |
| Phase 5 | 5（D5-01, D5-05, D5-07, D5-08, D5-09） | Yes — 最も恩恵大 |
| Phase 6 | 1（D6-03） | Yes |
| Phase 7 | 1（D7-02） | Yes |
| Phase 8 | 2（D8-02, D8-03） | Yes |
| Phase 9 | 0 | No — 単一エージェント（Audit Gate Lite） |
