...
description: >-
  feature-dev / debug-flow 共通の Audit Gate プロトコル。Phase 完了後の監査フロー、
...
| {smoke-test} | `feature-implementer` | {execute} の `feature-implementer` を再起動 |
...
{execute} 以降でコード変更が発生した場合、GitHub PR の「push fix -> CI -> re-review」サイクルを再現する。CI（{execute}/{smoke-test}/{doc-audit} Re-gate）だけでなく、レビュー自体も再実行する。
...
pipeline:
  {execute} → Audit → [{smoke-test}] → [{doc-audit}] → {review} → findings → fix
    → {execute} Re-gate → [{smoke-test} Re-gate] → [{doc-audit} Re-gate] → /code-review(re-review) → findings → fix
    → {execute} Re-gate → [{smoke-test} Re-gate] → [{doc-audit} Re-gate] → /code-review(re-review) → no findings
...
|-----------|----------|
| push | コード変更（fix 適用） |
| CI | {execute} Re-gate + {smoke-test} Re-gate + {doc-audit} Re-gate |
...
{review}: /code-review → findings → 修正適用
  ...
  |    |
  |    +-- Step 3: {smoke-test} Audit Gate 再実行（--smoke 有効時）
  |    |    +-- PASS -> Step 4
...
1. **正常終了**: {execute} Re-gate PASS -> {smoke-test} Re-gate PASS -> {doc-audit} Re-gate PASS -> re-review で findings なし -> {review} Audit Gate へ
...
- max_retries は {execute}/{smoke-test} の done-criteria ファイルに定義された値を使用
...
| {review} | 修正で UI が壊れる | {smoke-test} Re-gate |
