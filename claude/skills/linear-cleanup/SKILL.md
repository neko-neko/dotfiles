...
| シグナル | 判定 |
|---|---|
| 外部ソースでの議論内容・実際のスコープとチケットタイトルに乖離がある | タイトル変更候補 |
| チケットのスコープが当初から変化し、タイトルが実態を反映していない | タイトル変更候補 |

#### 期限未設定の検出

| シグナル | 判定 |
|---|---|
| 外部ソースで期限が合意されているがチケットに dueDate 未設定 | 期限設定候補 |
| description に期限の記述があるが dueDate 未設定 | 期限設定候補 |
